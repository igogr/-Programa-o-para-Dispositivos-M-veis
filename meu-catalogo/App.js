...
export default function App() {
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.subtitulo}>Filmes (Desenhos)</Text>
      {listaFilmes.map((filme, index) => (
        <Filme
          key={index}
          {...filme}
          onPress={() => Alert.alert("Filme", `Você clicou em ${filme.nome}`)}
        />
      ))}

      <Text style={styles.subtitulo}>Séries (Suspense)</Text>
      {listaSeries.map((serie, index) => (
        <Serie
          key={index}
          {...serie}
          onPress={() => Alert.alert("Série", `Você clicou em ${serie.nome}`)}
        />
      ))}
